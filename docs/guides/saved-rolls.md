...
- **The claim tracks the room's `pending_roll`, so it shrinks with it.** A reroll wave *removes* its targets, and the room then completes the roll naming only the survivors (issue #226) — a subset of what was claimed. `applyDiceRemoval` therefore filters removed ids out of the claim (and nulls it once empty, since the room drops a roll with nothing left and announces nothing). Left at full width, the claim would stop matching that completion and the roll would land twice: once from `finishSavedRollWaves`, once from the message the claim existed to swallow. The comparison stays an *exact* set match on purpose — matching a subset would let a genuinely different, smaller roll be swallowed instead.
...
A wave sequence's own claimed dice are exempt from the *orphan mark*, since that is the reroll wave discarding its dice and `finishSavedRollWaves` owns that row. They are **not** exempt from the claim shrink above: the room still shrinks its `pending_roll` and still completes the roll from the survivors, so the claim has to follow it or that completion arrives unrecognised. Exemption from the mark and tracking of the claim are separate decisions taken in the same update.
