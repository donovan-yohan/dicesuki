...
| Runtime GLB delivery | External release source lock, production sidecars, `runtime-assets.json` | Bounded `/public/dice/` GLBs and thumbnails; lazy local/remote table rendering | `scripts/runtime-dice-assets/`, `npm run check:runtime-dice-assets`, `docs/guides/runtime-dice-assets.md` |
...
| Local inventory and gameplay | `src/store/useInventoryStore.ts`, `src/lib/diceSpawner.ts` | React/R3F solo experience; retained legacy device-local dice may reload from IndexedDB, but customer uploads are retired | Co-located Vitest tests, `npm test`, `npm run build` |
| Controlled dice authoring and promotion | `docs/guides/dice-set-authoring.md`, `src/config/collectibleCatalogSource.json`, `supabase/catalog/editions/` | Operator-authored dice become immutable bundled catalog assets | Workshop validation, catalog preparation, immutable-history and runtime-asset checks |
