...
| Local inventory and gameplay | `src/store/useInventoryStore.ts`, `src/lib/diceSpawner.ts` | React/R3F solo experience; v6 removes retired customer-uploaded dice while immutable catalog assets remain available | Co-located Vitest tests, `npm test`, `npm run build` |
