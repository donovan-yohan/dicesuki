...
const themes = [
  {
    ...
    environmentDiceNames: ['Hearthwood D6', 'Acorn Compass D10', 'Elder Canopy D20'],
  },
  {
    id: 'dark-dungeon',
    ...
    environmentDiceNames: ['Iron Vault D6', 'Gaoler Key D10', 'Dread Gate D20'],
  },
  {
    id: 'cyberpunk-box',
    ...
    environmentDiceNames: ['Neon Grid D6', 'Cipher Core D10', 'Overdrive D20'],
  },
]
...
try {
  for (const theme of themes.filter((entry) => !captureTheme || entry.id === captureTheme)) {
    ...
    for (const dieName of environmentDiceNames) {
      await environmentPage.getByRole('button', { name: 'Inventory' }).click()
      const search = environmentPage.getByPlaceholder('Search dice...')
      await search.fill(dieName)
      await environmentPage.getByRole('button', { name: new RegExp(dieName) }).click()
      await environmentPage.getByRole('button', { name: /Spawn to Table/ }).click()
