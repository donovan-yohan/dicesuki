...
/**
 * Step 3 of the themed dice pipeline: snap the authored ImageGen art back onto
 * the canonical UV islands and bleed it past every island edge.
 *
 * ImageGen returns art that is *visually* on-template but drifts by a few
 * pixels per island. Registration re-projects each island's painted region onto
 * the exact polygon recorded in the canonical manifest, so UVs baked from the
 * manifest land on the right pixels and seams do not show background.
 *
 * Resurrected from commit 7393d112c5e062570ec7caf37970206c4d05c08c. Adapted to
 * current `main`:
 * - Manifests come from `<workshop>/templates/<shape>/manifest.json`
 *   (canonical-contract-v2) rather than the removed
 *   `public/artist-resources/.../<shape>-mesh-uv-manifest.json`.
 * - Added `--theme` selection so a single set can be registered.
 * - Themes with no raw art present are reported instead of throwing, so the
 *   command is usable while only one set is being authored.
 *
 * Usage:
 *   node scripts/imagegen-uv/register-theme-atlases.mjs [--theme fantasy-earth] [--out DIR]
 */
...
/**
 * D20 islands are tiny triangles packed tight; they need more bleed than the
 * larger island shapes or 1024px runtime downscaling reveals seams.
 */
const D20_BLEED_SCALE = 1.1

export async function registerThemeAtlases(options = {}) {
  ...
  try {
    for (const theme of themes) {
      for (const shape of THEME_WORKSHOP_SHAPES) {
        ...
        await mkdir(path.dirname(paths.atlas), { recursive: true })
        await writeFile(paths.atlas, buffer)
        registered.push(`${theme.id}/${shape}`)
      }
    }
  } finally {
    await page.close()
  ...
  return { registered, skipped, root }
}
...
async function main() {
  ...
  if (result.registered.length === 0) {
    throw new Error(
      `No raw ImageGen atlases found under ${result.root}. `
      + 'Complete the art pass first (see docs/guides/dice-set-authoring.md).',
    )
  }
  console.log(`Registered and edge-bled ${result.registered.length} atlas(es): ${result.registered.join(', ')}`)
  if (result.skipped.length > 0) {
    console.log(`Skipped ${result.skipped.length} shape(s) with no raw art yet: ${result.skipped.join(', ')}`)
  }
}
