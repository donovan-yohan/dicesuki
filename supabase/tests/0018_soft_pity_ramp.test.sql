...
insert into public.pull_banner_families (id) values ('slice6-null-ramp');
...
select 'slice6-null-ramp@1', pull_count, cost
...
select 'slice6-null-ramp@1', tier_id, tier_rank, weight_units, duplicate_dust
...
begin
  if (select count(*) from public.pull_banner_offers
      where banner_version_id = 'slice6-null-ramp@1') <>
     ...
     (select max(banner_version) from public.pull_banner_versions
      where banner_family_id = 'slice6-null-ramp') <> 1 then
...
begin
  if not exists (
    select 1
    from public.pull_banner_versions
    where id in ('earned-collection-001@1', 'slice6-null-ramp@1')
