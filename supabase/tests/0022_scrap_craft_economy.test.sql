...
insert into public.pull_banner_families (id) values ('slice9-hold-stars');
...
select 'slice9-hold-stars@1', pull_count, cost
...
select 'slice9-hold-stars@1', tier_id, tier_rank, weight_units, duplicate_dust
...
begin
  if (select count(*) from public.pull_banner_offers
      where banner_version_id = 'slice9-hold-stars@1') <>
     ...
     (select max(banner_version) from public.pull_banner_versions
      where banner_family_id = 'slice9-hold-stars') <> 1 then
