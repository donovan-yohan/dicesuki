-- Migration: 0021_pull_copy_grant_rework
-- Monetization economy spec section 6.1 delta 11.
--
-- Pull ownership now means live dice-copy count. Every committed result grants
-- a copy, duplicate results additionally credit their sealed Dust, and the
-- ever-owned latch is returned for first-copy presentation.

-- Copy of the canonical 0018 preparation body. Only the three ownership
-- predicates differ: live dice copies replace entitlement booleans.
--
-- Spec section 1.6 conservative zero-target rule: when every selected-featured
-- item has a live copy, selected_item stays NULL, so neither selected hard pity
-- nor soft pity arms. The existing epic/rare/base resolution order then applies.
...
-- Committed-only reveal builder. is_first_copy comes from the immutable grant
-- row identified by the same session/result idempotency key used at commit.
    ...
    'results', coalesce((
      select jsonb_agg(
        jsonb_build_object(
          ...
          'is_first_copy', (
            select copies.is_first_copy
            from public.dice_copies as copies
            where copies.user_id = target_session.user_id
              and copies.grant_idempotency_key =
                'pull-copy-grant:' || target_session.id::text ||
                ':result:' || results.result_position::text
          ),
...
comment on function private.get_committed_pull_reveal_for_user(uuid, uuid) is
  'Committed-only owner-scoped reveal builder containing sealed results, copy-latch flags, nonces, RNG seed, and commitment fields.';
...
-- The transition append releases this session's ownership hold before every
-- sealed result is granted through the canonical dice-copy boundary.
...
begin
  ...
  -- guarantee, copy, and ledger operation in this engine.
  target_account := private.lock_wallet_account(p_user_id);
...
comment on function private.commit_pull_session_for_user(uuid, uuid) is
  'Private exactly-once pull settlement engine. Locks wallet_accounts first, appends committed before debit, advances guarantees, grants one copy per sealed result, credits duplicate Dust, and returns a verifiable reveal.';

revoke all on function private.commit_pull_session_for_user(uuid, uuid)
  from public, anon, authenticated, service_role;

-- Both entitlement inserts (the direct-purchase rail) and dice-copy inserts
-- (the pull/craft/reward ownership rail) keep their existing account lock and
-- are rejected while any unexpired non-terminal ownership snapshot is sealed.
...
comment on function private.preserve_pull_ownership_snapshot() is
  'Account-wide live-hold guard for entitlement and dice-copy grants. Direct purchases remain guarded; committed/cancelled and expired sessions do not block grants.';

revoke all on function private.preserve_pull_ownership_snapshot()
  from public, anon, authenticated, service_role;

create trigger dice_copies_preserve_pull_snapshot
  before insert on public.dice_copies
  for each row execute function private.preserve_pull_ownership_snapshot();

-- Copy of the 0020 marker primitive with one new safety check: a new scrap is
-- forbidden while any live pull hold depends on the account's copy snapshot.
-- An exact replay returns before this check because it performs no mutation.
...
begin
  ...
  decision_at := clock_timestamp();
  -- Account-wide (not family-scoped) to mirror preserve_pull_ownership_snapshot.
  -- With multiple banner families this is deliberately conservative: a live
  -- hold in any family blocks a scrap that could alter another sealed family.
  if exists (
    select 1
    from public.pull_sessions as sessions
    where sessions.account_id = target_account.id
    ...
    raise exception 'Dice-copy scraps are paused while a prepared pull hold is active'
      using errcode = '55000';
  end if;
...
comment on function private.record_dice_copy_scrap(uuid, uuid, text) is
  'Idempotent dice-copy scrap marker. Account-wide live pull holds reject new scraps with SQLSTATE 55000 so sealed ownership predicates remain valid.';
