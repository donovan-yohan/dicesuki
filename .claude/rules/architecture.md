...
- [Shared-ADR-005] Every die a client spawns MUST carry a presentation block; a die with no owned identity MUST spawn as a *basic* die (`basic: true`, white body, black numerals, no `inventoryDieId`/`setId`/`rarity` — see `src/lib/basicDice.ts`). The owner-colour fallback in `MultiplayerDie` is reachable only for dice originated by another or older client. <!-- hand-synced pending /adr:update regeneration; source: Shared-ADR-005 Amendment 1 (2026-07-28), superseding "Generic anonymous dice MUST spawn without any presentation block" -->
- [Shared-ADR-005] Basic dice MUST stay invisible to ownership-derived surfaces (toolbar available count, set completion, selling, stats), all of which key on `inventoryDieId`. <!-- hand-synced pending /adr:update regeneration; source: Shared-ADR-005 Amendment 1 (2026-07-28) -->
